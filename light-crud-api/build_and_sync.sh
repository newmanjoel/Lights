...
if [ "$release" = true ]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "Restarting the light service"
        ssh "pi@192.168.2.24" "sudo systemctl restart lights.service"
        echo "Restarted the light service"
    fi
fi
# if [ "$pi_2w" = true ]; then
#     cargo build --release --target aarch64-unknown-linux-gnu
#     if [ $? -eq 0 ]; then
#         rsync -avz --delete /home/joel/GH/Lights/light-crud-api/target/aarch64-unknown-linux-gnu/release/light-crud-api lights@192.168.2.33:/home/lights/light-crud-api
#     fi
# else
